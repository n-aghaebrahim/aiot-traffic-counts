...
def main(datafile):
    ...
    if config["model_name"] == 'conv_lstm':
        model_c = model.create_conv_lstm_model(config["n_steps"], config["n_features"], config["filter_num"], config["kernel_size"], config["pool_size"], config["lstm_units"], config["dense_units"], config["dropout_rate"], config["learning_rate"])
    else:
        model_c = model.create_lstm_model((X_train.shape[1], 1))  # Fallback to a default LSTM model

    # Model training and prediction
    history = train.train_model(model_c, X_train, y_train, epochs=config["epochs"], batch_size=config["batch_size"], validation_split=config["validation_split"], verbose=config["verbose"])
